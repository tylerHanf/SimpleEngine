...
void GuiContext::RenderGui(MouseRay* pointer, EntityHandler* entities, glm::mat4 mMat) {
  ...
  glm::vec3 rayOrig = pointer->getOrigin();
  glm::vec3 rayDir = pointer->getDirection();
  ...
  ImGui::Begin("Data");
  ImGui::Text("Ray worldspace origin: <%f, %f, %f>", rayOrig.x, rayOrig.y, rayOrig.z);
  ImGui::Text("Ray worldspace direction: <%f, %f, %f>", rayDir.x, rayDir.y, rayDir.z);
